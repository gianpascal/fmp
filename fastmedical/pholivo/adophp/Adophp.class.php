...
//		$this->dbdriver = 'POSTGRES';
	...
	public function ConnectionClose()
	{
		try
		{
			...
			{
				case "POSTGRES":
					if(isset($this->RecordSet) && $this->RecordSet !== FALSE) pg_free_result($this->RecordSet);
					@pg_close($this->pConnection);
				break;
				case "MSSQL":
					if(isset($this->RecordSet) && $this->RecordSet !== FALSE && $this->RecordSet!=1) @mssql_free_result($this->RecordSet);
	...
	public function GetBeginSchema($dbdriver,$pSchema) {
		if(trim($pSchema) != "")	return $pSchema;
		else
		switch(strtoupper($dbdriver))
		{
			case "POSTGRES":	return 	"public";	break;
			case "MSSQL":		return 	"dbo";		break;
	...
	public function SetParameterSP($pName, $pValue,$pType="VARCHAR",$pIsOutput=false,$pIsNull=false,$plength=NULL) {
		...
		switch(strtoupper($pType))
		{
                    ...
                    case "VARCHAR":case "DATE":	case "TEXT":default:
                        if ($pValue=="")				$pValue="''";
                            ...
                            $pValue = $pValue; $pName = substr($pName,1,strlen($pName)-2);
                        }
                        else
                            $pValue = "'$pValue'";
                    break;
		...
		switch(strtoupper($this->dbdriver))
		{
                    ...
                    case "MSSQL":
                        switch(strtoupper($pType))
                        {
                            case "INT":	case "INTEGER":     $pType="SQLINT4";	 break;
                            case "CHAR":                    $pType="SQLCHAR";	 break;
                            case "VARCHAR":                 $pType="SQLVARCHAR"; break;
                            case "TEXT":                    $pType="SQLTEXT";	 break;
                            case "NUMERIC":case "FLOAT":    $pType="SQLFLT8";	 break;
                            case "DATETIME":case "DATE":    $pType="SQLDATETIME";break;
                        }
                    default:
                    break;
		}
	...
	public function SetFieldsOrder($pName, $pOrder="ASC") {
		if(strtoupper($pOrder) == 'DESC') 	$ord = " DESC";
		else												$ord = " ASC";
   ...
   public function InitSp(){
	...
	private function Prepare_Sql() {
		...
		{
			case "POSTGRES":
				if(trim($this->pFieldsSelect)=="")			$query="SELECT * ";
				else										$query="SELECT ".$this->pFieldsSelect;
				$query.=" FROM ".$this->pTables;
				if(trim($this->pFieldsCondition)!="")		$query.=" WHERE ".$this->pFieldsCondition;
				if(trim($this->pFieldsOrder)!="")			$query.=" ORDER BY ".$this->pFieldsOrder." ".$this->pTypeOrder;
		...
		switch(strtoupper($this->dbdriver))
		{
			case "POSTGRES":
                                ...
                                if($this->pNameStoreProcedure=='fn_mante_persona_natural')
                                {
                                    ...
                                    foreach($this->pParameter as $Value)    $query .= $Value . ",";
                                    $len = strlen($query);
                                    if(count($this->pParameter)>0) $len -= 1;
				...
				if(trim($spcondition)!="")  $query.=" WHERE ".$spcondition;
				if(trim($this->pFieldsOrder)!="")  $query.=" ORDER BY ".$this->pFieldsOrder." ".$this->pTypeOrder;
				$this->pSql=$query;
                                break;
			case "MSSQL":	//echo $this->pSP;
                            if($this->pSP=="PROCEDURE")
                            {
                                ...
                                $len = strlen($query);
                                if(count($this->pParameter)>0) $len -= 1;	$query = substr($query, 0, $len);
                            }
	...
	public function Move($MoveRow) {
		...
		if(isset($this->aRecords)) {
			switch($MoveRow) {
				case 1:		$this->pRowCursor= 0;	break;
				case 2:		$this->pRowCursor-=1;	break;
				case 3:		$this->pRowCursor+=1;	break;
				case 4:		$this->pRowCursor=$this->pNumRows - 1 ;	break;
				default:	$this->pRowCursor+=1;	break;
			}

			if($this->pRowCursor<0 or $this->pRowCursor>($this->pNumRows-1))	$result =false;
			$this->pRow =false;
			if($result){
				$arreglo=array();
				$arreglo2=array();
	...
	public function ExecuteSPArray($outputType="Message"){
        ...
        public function executeSPArrayX(){
            ...
            elseif(strtoupper($this->dbdriver)=="MSSQL"){
                //$array = mssql_fetch_array($this->RecordSet,0);
                //var_dump($array);
                //echo "-----------------------";
	...
	public function _Execute(){
		...
		for($i=0;$i<$this->pNumRows;$i++)
		{
                ...
                break;
                case "MSSQL":
                    if(trim($this->pFieldsSelect)!="" and trim($this->pFieldsSelect)!="*" and $this->pNameStoreProcedure)	$sw=1;
                    if($this->pRowsPage != "ALL")
                    {
                        $this->setTotalPages($this->pRowsPage);
			$k=0;
	...
	public function GetSql(){
		if(!$this->pSql or $this->pSql=="")
		{
			if(!$this->pNameStoreProcedure or $this->pNameStoreProcedure=="")		{$this->Prepare_Sql();	}
			else																	{$this->Prepare_StoreProcedure();	}
		}
		return $this->pSql;
	}
