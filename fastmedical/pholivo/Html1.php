...
class Tabla1
{
    ...
    public function __construct($_arrayCabecera, $_numFilas, $_arrayFilas = '', $_estiloTabla = '', $_estiloFila = '', $_estiloFilaAlterna = '', $_estiloFilaSeleccionada = '', $_arrayEventosJS = '', $_oyenteEventosJS = '', $_indClave = '', $_arrayTipos = '', $_indiceEstados = '', $_arrayColorEstado = '')
    {
        self::$idHijo++;
    ...
    public function getCabeceraHTML($_arrayCabecera = '')
    {
        ...
        foreach ($_arrayCabecera as $i => $valor) {
            if ($valor == "checkbox") {
                $valor = "<input type=\"checkbox\" onclick=\"tablaPHP.seleccionarChecks(this,event,$k);\"/>";
            }
    ...
    {
        if (empty($this->arrayCabecera)) {
            return "Error: La propiedad arrayFilas  de Tabla1 no contiene un array";
        }
...
class Check
{
    ...
    {
        if ((count($this->arrayEventos) != count($this->arrayEscuchador)) || (count($this->arrayEscuchador) <= 0)) {
            return "";
        }
...
class ButtonDelete
{
    ...
    private $icono;
    private $funcion = '';
    public function __construct($id = '', $name = '', $style = '', $class = '', $icono = '', $funcion = '')
    {
        $this->id = $id;
        $this->name = $name;
        $this->style = $style;
