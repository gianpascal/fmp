...
$_(data.id).innerHTML=objAjax.responseText+$_(data.id).innerHTML;else if(data.iner==1)
$_(data.id).innerHTML=$_(data.id).innerHTML+objAjax.responseText;else
$_(data.id).innerHTML=objAjax.responseText;break;}
case 2:{var objxml=new isiXML(objAjax.responseXML);data.id(objxml.Query(data.query),objAjax);break;}
case 3:{if(data.iner==0)
$_(data.id).value=objAjax.responseText+$_(data.id).value;else if(data.iner==1)
$_(data.id).value=$_(data.id).value+objAjax.responseText;else
$_(data.id).value=objAjax.responseText;break;}
case 4:{var obj=$_(data.id);var objxml=new isiXML(objAjax.responseXML);var resultado=objxml.Query('SELECT node.value, atrib.value');var total=obj.options.length;if(total > 0)
...
function(idform,id,iner){var form=$_(idform);this.Request({url:this.Url(form.action),
method:form.method,
id:id,
...
function(url,id,w,h){var temp=new Image();var parent=this;this.working=true;this.Cargador(true);temp.onload=function(){if(parent.aborta){$_(id).src=temp.src;if(w)$_(id).width=w;if(h)$_(id).height=h;}
...
function(nombre,valor){resultado+=nombre+'='+encodeURIComponent(valor);if((i+1)< obj.length)
...
$_(id).style.visibility=(onoff)? 'visible':'hidden';};this.Cargador(false);if(typeof XMLHttpRequest!='undefined')
