dhtmlXGridObject.prototype.lL=function(fl,zU,xy,uF,sG,qn){
    this._pgn_parentObj=typeof(uF)=="string"?document.getElementById(uF):uF;
...
dhtmlXGridObject.prototype.changePageRelative=function(ind){
    this.changePage(this.ew+ind);
};

dhtmlXGridObject.prototype.changePage=function(eZ){
    if(arguments.length==0)eZ=this.ew||0;
    eZ=parseInt(eZ);
    eZ=Math.max(1,Math.min(eZ,Math.ceil(this.aD.length/this.cu)));
    if(!this.callEvent("onBeforePageChanged",[this.ew,eZ]))return;
    this.ew=parseInt(eZ);
    this._reset_view();
    this._fixAlterCss();
    this.callEvent("onPageChanged",this.ajA());
};
...
dhtmlXGridObject.prototype._page_skin_update=function(name){
    if(!this.pagesInGroup)this.pagesInGroup=Math.ceil(Math.min(5,this.aD.length/this.cu));
    var Cg=Math.ceil(this.aD.length/this.cu);
    if(Cg&&Cg<this.ew)return this.changePage(Cg);
    if(this.aW&&this._pgn_skin)this._pgn_skin.apply(this,this.ajA());
};
...
dhtmlXGridObject.prototype._pgn_default=function(page,start,end){
    if(!this.pagingBlock){
        ...
        if(this._pgn_recInfoParentObj&&this.sG)this._pgn_recInfoParentObj.appendChild(this.pq);
...
dhtmlXGridObject.prototype._pgn_block=function(sep){
    var start=Math.floor((this.ew-1)/this.pagesInGroup)*this.pagesInGroup;
    var max=Math.min(Math.ceil(this.aD.length/this.cu),start+this.pagesInGroup);
    var str=[];
    for(var i=start+1;i<=max;i++)if(i==this.ew)str.push("<a class='dhx_not_active'><b>"+i+"</b></a>");else str.push("<a onclick='this.grid.changePage("+i+");return false;'>"+i+"</a>");return str.join(sep);
};

dhtmlXGridObject.prototype._pgn_link=function(mode,ac,ds){
    if(mode=="prevpages"||mode=="prev"){
        if(this.ew==1)return ds;
    ...
    if(mode=="nextpages"||mode=="next"){
        if(this.aD.length/this.cu<=this.ew)return ds;
        if(this.aD.length/(this.cu*(mode=="next"?'1':this.pagesInGroup))<=1)return ds;
        return '<a onclick=\'this.grid.changePageRelative('+(mode=="next"?'1':'this.grid.pagesInGroup')+');return false;\'>'+ac+'</a>'
        };

    if(mode=="current"){
        var i=this.ew+(ac?parseInt(ac):0);
        if(i<1||Math.ceil(this.aD.length/this.cu)<i)return ds;
        return '<a '+(i==this.ew?"class='dhx_active_page_link' ":"")+'onclick=\'this.grid.changePage('+i+');return false;\'>'+i+'</a>'
        };
...
dhtmlXGridObject.prototype._pgn_template_compile=function(template){
    template=template.replace(/\[([^\]]*)\]/g,function(a,b){
        ...
        switch(b[0]){
            ...
            case "to":
                return '"+arguments[2]+"';
            case "current":case "prev":case "next":case "prevpages":case "nextpages":
...
dhtmlXGridObject.prototype._pgn_bricks=function(page,start,end){
    var YV=(this.skin_name||"").split("_")[1];
    var sfx="";
    if(YV=="light"||YV=="modern"||YV=="skyblue")sfx="_"+YV;
    ...
    if(this.aD.length>0){
        ...
        for(var i=1;i<=this.pagesInGroup;i++){
            ...
            if(pageNumber>Math.ceil(this.aD.length/this.cu))break;
    ...
    if(Math.ceil(page/this.pagesInGroup)*this.pagesInGroup<Math.ceil(this.aD.length/this.cu)){
        var pageCont=document.createElement("DIV");
        var pageBox=document.createElement("DIV");
        pageCont.className="dhx_page"+sfx;
...
if(this.aD.length>0&&this.sG)this.pagerElAr["pagerInfoBox"].innerHTML=this.i18n.paging.records+(start+1)+this.i18n.paging.to+end+this.i18n.paging.of+this.aD.length;
...
if(this.rC[1]){
    if(!this.iD())this.bk.setItemText('results',this.i18n.paging.notfound);else this.bk.setItemText('results',"<div style='width:100%;text-align:center'>"+this.i18n.paging.records+(start+1)+this.i18n.paging.to+end+"</div>");
};

if(this.rC[3])this.bk.setItemText("perpagenum","<div style='width:100%;text-align:right'>"+this.cu.toString()+" "+this.i18n.paging.perpage+"</div>");
    this.callEvent("onPaging",[]);
};
...
if(this.rC[0]){
    this.bk.addButton("leftabs",NaN,"",this.eg+'ar_left_abs.gif',this.eg+'ar_left_abs_dis.gif');
    this.bk.setWidth("leftabs","20");
    this.bk.addButton("left",NaN,"",this.eg+'ar_left.gif',this.eg+'ar_left_dis.gif');
...
if(this.rC[0]){
    this.bk.addButton("right",NaN,"",this.eg+'ar_right.gif',this.eg+'ar_right_dis.gif');
    this.bk.setWidth("right","20");
    this.bk.addButton("rightabs",NaN,"",this.eg+'ar_right_abs.gif',this.eg+'ar_right_abs_dis.gif');
...
if(arr=this.rC[3]){
    ...
    for(var k=0;k<arr.length;k++)this.bk.addListOption('perpagenum','perpagenum_'+arr[k],NaN,"button",arr[k]+" "+this.i18n.paging.perpage);
    this.bk.setWidth("perpagenum","135");
};
