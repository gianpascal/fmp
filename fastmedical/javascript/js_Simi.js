...
function parpadear_mensaje() {
	...
	if (ObtenerValorObjeto("sw_mensaje") == 1) {
		if (ObtenerValorObjeto("mensaje", "style.display") == "none")
			ColocarValorObjeto("mensaje", "inline", "style.display");
		else {
			ColocarValorObjeto("mensaje", "none", "style.display");
			cnt_mensaje = cnt_mensaje + 1;
			if (cnt_mensaje == 21)
				ColocarValorObjeto("sw_mensaje", 0);
		}
	}
	else
		cnt_mensaje = 0;
	if (ObtenerValorObjeto("sw_mensaje") == 1) {
		msje = ObtenerValorObjeto("mensaje");
		ColocarValorObjeto("mensaje", "");
		ColocarValorObjeto("sw_mensaje", 0);
		if (msje != undefined) alert(msje);
	}
	setTimeout('parpadear_mensaje()', 1000);
	myRand = parseInt(Math.random() * 999999999999999);
	//myajax4.Link('estado_sesion.php?rand='+myRand,'Mensaje');	
	if (ExisteObjeto("cerrar_sesion")) {
		location.href = "../../cerrar_sesion.php?msn=" + ObtenerValorObjeto("cerrar_sesion");
	}
}

function CerrarSesion() {
	location.href = "../../cerrar_sesion.php?msn=";
}



function CargarVentana(vformname, vtitle, file, vwidth, vheight, vcenter, vresizable, vstyle, vopacity, veffect, vposx1, vposx2, vposy1, vposy2) {
...
function CargarVentanaFormulario(vFormaAbrir, vformname, vtitle, file02, vwidth, vheight, vcenter, vresizable, vmodal, vstyle, vopacity, veffect, vposx1, vposx2, vposy1, vposy2, file01, vfunctionjava) {
	//alert(vFormaAbrir+'---'+vformname+'---'+vtitle+'---'+file02+'---'+vwidth+'---'+vheight+'--\n-'+vcenter+'---'+vresizable+'---'+vmodal+'---'+vstyle+'---'+vopacity+'--\n-'+veffect+'---'+vposx1+'---'+vposx2+'---'+vposy1+'-\n--'+vposy2+'---'+file01)
	//alert("asdfd");
	//	vresizable,vstyle,vefecto,vmodal,opacity
	...
	if (vstyle == undefined || vstyle == 0) vstyle = "alphacube";
	if (veffect == veffect || veffect == 0) veffect = "popup_effect1";
	...
	if (trim(file01) == "") {
		if (!ExisteObjeto("Div_" + vformname)) {
			...
			if (vFormaAbrir == "_self") {
				vidfrm = "Frm_" + vformname;
				document.getElementById("Contenido").innerHTML = "<div id='" + vidfrm + "'></div>";
				temp = "" + file02;
				ultimocaracter = temp.substring(temp.length - 1, temp.length);
				switch (ultimocaracter) {
					case 'p': fileallamar = file02 + '?rand=' + myRand; break;
					case '?': fileallamar = file02 + 'rand=' + myRand; break;
					default: fileallamar = file02 + '&rand=' + myRand; break;
				}
				myajax.Link(fileallamar, vidfrm, 0, vfunctionjava);
			}
			else if (vFormaAbrir == "EXTERNO") {
				temp = "" + file02;
				ultimocaracter = temp.substring(temp.length - 1, temp.length);
				parametros = "id_usuario=" + ObtenerValorObjeto('id_usuario') + "&usuario=" + ObtenerValorObjeto('usuario') + "&id_sistema=" + ObtenerValorObjeto('id_sistema') + "&id_caja=" + ObtenerValorObjeto('id_caja') + "&ip=" + ObtenerValorObjeto('ip');
				switch (ultimocaracter) {
					case 'p': fileallamar = file02 + '?rand=' + myRand + '&' + parametros; break;
					case '?': fileallamar = file02 + 'rand=' + myRand + '&' + parametros; break;
					default: fileallamar = file02 + '&rand=' + myRand + '&' + parametros; break;
				}
				location.href = decodeURIComponent(file02) + '?' + parametros;
			}
			else if (vFormaAbrir == "VENTANA") {
				//var effect = new PopupEffect(html, {className:veffect});
				var vid = "Div_" + vformname;
				vidfrm = "Frm_" + vformname;
				//                                        var vzindex=parseInt(ObtenerValorObjeto("zindex"))+1;
				//					ColocarValorObjeto("zindex",vzindex)
				var vzindex = 100;
				var win;
				...
				//alert(fileallamar);
				console.log(fileallamar);
				console.log(vidfrm);
				console.log(vfunctionjava);
				//myajax.Link(fileallamar, vidfrm, 0, vfunctionjava);
		...
		var vzindex = parseInt(ObtenerValorObjeto("zindex")) + 1;
		ColocarValorObjeto("zindex", vzindex)
		var win = new Window({ id: vid, className: 'alphacube', title: vtitle, width: vwidth, height: vheight, zIndex: vzindex, opacity: 1, resizable: true });
		win.getContent().innerHTML = "<div id='" + vidfrm + "'></div>";
		win.setDestroyOnClose();
		if (vcenter == true || vcenter == 1)
			win.showCenter(true);
		...
		win.toFront();
		parametros = '&vformaabrir=' + vFormaAbrir + '&vformname=' + vformname + '&vtitle=' + vtitle + '&vwidth=' + vwidth + '&vheight=' + vheight + '&vposx1=' + vposx1 + '&vposy1=' + vposy1 + '&vposx2=' + vposx2 + '&vposy2=' + vposy2 + '&vresizable=' + vresizable + '&vstyle=' + vstyle + '&veffect=' + veffect + '&vmodal=' + vmodal + '&vopacity=' + vopacity + '&vcenter=' + vcenter + '&vtitle=' + vtitle + "&vfunctionjava=" + vfunctionjava;
		myajax.Link(file01 + '?file02=' + encodeURIComponent(file02) + '&rand=' + myRand + parametros, vidfrm);
	}
...
function CargarVistaImpresion(vformname, vtitle, file, vwidth, vheight, vcenter, vresizable, vstyle, vopacity, veffect, vposx1, vposx2, vposy1, vposy2) {
	CargarImpresionFormulario("VENTANA", vformname, vtitle, file, vwidth, vheight, vcenter, vresizable, true, vstyle, vopacity, veffect, vposx1, vposx2, vposy1, vposy2, '', '')
}

function CargarImpresionFormulario(vFormaAbrir, vformname, vtitle, file02, vwidth, vheight, vcenter, vresizable, vmodal, vstyle, vopacity, veffect, vposx1, vposx2, vposy1, vposy2, file01, vfunctionjava) {
	//alert(vFormaAbrir+'---'+vformname+'---'+vtitle+'---'+file02+'---'+vwidth+'---'+vheight+'--\n-'+vcenter+'---'+vresizable+'---'+vmodal+'---'+vstyle+'---'+vopacity+'--\n-'+veffect+'---'+vposx1+'---'+vposx2+'---'+vposy1+'-\n--'+vposy2+'---'+file01)
	//alert("asdfd");
	//	vresizable,vstyle,vefecto,vmodal,opacity
	...
	if (trim(file01) == "") {
		if (!ExisteObjeto("Div_" + vformname)) {
			...
			if (vFormaAbrir == "INTERNO") {
				...
				ColocarValorObjeto("zindex", vzindex)
				if (vmodal == true || vmodal == 1)
					var win = new Window({ id: vid, className: vstyle, title: vtitle, width: vwidth, height: vheight, zIndex: vzindex, opacity: vopacity, resizable: false, closable: false, minimizable: false, maximizable: false });
				else
					var win = new Window({ id: vid, className: vstyle, title: vtitle, width: vwidth, height: vheight, resizable: false, closable: false, minimizable: false, maximizable: false });
				win.getContent().innerHTML = "<div id='" + vidfrm + "'></div>";
				...
				if (vcenter == true || vcenter == 1)
					...
					win.show(vmodal);
				win.setConstraint(true, { left: vposx1, right: vposx2, top: vposy1, bottom: vposy2 })
				win.toFront();
				temp = "" + file02;
				ultimocaracter = temp.substring(temp.length - 1, temp.length);
				switch (ultimocaracter) {
		...
		var vzindex = parseInt(ObtenerValorObjeto("zindex")) + 1;
		ColocarValorObjeto("zindex", vzindex)
		var win = new Window({ id: vid, className: 'alphacube', title: vtitle, width: vwidth, height: vheight, zIndex: vzindex, opacity: 1, resizable: false, closable: false, minimizable: false, maximizable: false });
		win.getContent().innerHTML = "<div id='" + vidfrm + "'></div>";
		win.setDestroyOnClose();
		if (vcenter == true || vcenter == 1)
			win.showCenter(true);
...
function CargarBuscador(var1, titulo, funcionjava, archivophp, opcion, parametro) {
	...
	if (key == 45 || var1 == 13) {
		...
		js_abrirventana("busquedas/buscar_gral.php?" + parameter, '', 'no', 'no', 'no', 'no', 'no', 'no', 'no', 500, 400, 'no');
	}
}
			...
			switch (NombreFormulario) {
				...
				case 'p_cargaEXCEL':
					if (document.getElementById('is_excel') != null && document.getElementById('is_excel').value == "S") {
						...
						js_abrirventana("presupuesto/reporte/excel/carga_excel.php?" + parameter, '', 'no', 'no', 'no', 'no', 'no', 'no', 'no', 500, 400, 'no');
					}
					else {
				...
				case 'p_cargaEXCELplan':
					if (document.getElementById('is_excel') != null && document.getElementById('is_excel').value == "S") {
						var parameter = "cod_user=" + document.getElementById('cod_user').value;
						js_abrirventana("presupuesto/plandetrabajo/excel/carga_excel.php?" + parameter, '', 'no', 'no', 'no', 'no', 'no', 'no', 'no', 500, 400, 'no');
					}
					else {
						alert("Debe Generar el Reporte, para poder EXPORTARLO\n en Formato EXCEL");
						return false;
...
function Grabar_Usuario_Pass(vParam, modo) {
	...
	if (document.getElementById("nuevo_pass").value != document.getElementById("re_pass").value) {
		...
		myajax.Link('../../ccontrol/control/control.php?p1=mante_paswd&' + vParam, 'Contenido');
}
